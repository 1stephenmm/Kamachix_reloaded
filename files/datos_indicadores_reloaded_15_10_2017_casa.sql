...
--
-- Definition for function fun_actua_formacion (OID = 25219) : 
--
...
LANGUAGE plpgsql;
--
-- Definition for function fun_actua_relacion (OID = 25220) : 
--
...
LANGUAGE plpgsql;
--
-- Definition for function fun_delete_cohort (OID = 25221) : 
--
...
LANGUAGE plpgsql;
--
-- Definition for function fun_delete_period (OID = 25222) : 
--
...
LANGUAGE plpgsql;
--
-- Definition for function Actua_Formacion_Dep (OID = 25223) : 
--
...
LANGUAGE plpgsql;
--
-- Definition for function Fun_Actua_KPI_Form (OID = 25224) : 
--
...
LANGUAGE plpgsql;
--
-- Definition for function fun_actua_satisfaccion (OID = 25225) : 
--
...
LANGUAGE plpgsql;
--
-- Definition for function Actua_Estudiantes_Dep (OID = 25226) : 
--
...
LANGUAGE plpgsql;
--
-- Definition for function Fun_Actua_KPI_Est_Dep (OID = 25227) : 
--
...
LANGUAGE plpgsql;
--
-- Structure for table KPI_Acreditacion (OID = 25228) : 
--
SET search_path = "Datawarehouse", pg_catalog;
...
WITH (oids = false);
--
-- Structure for table KPI_Desercion_Cohorte (OID = 25232) : 
--
...
WITH (oids = false);
--
-- Structure for table KPI_Desercion_Periodo (OID = 25236) : 
--
...
WITH (oids = false);
--
-- Structure for table KPI_Formacion (OID = 25245) : 
--
...
WITH (oids = false);
--
-- Structure for table KPI_Nivel_Satisfaccion (OID = 25249) : 
--
...
WITH (oids = false);
--
-- Structure for table KPI_Relacion_Docentes (OID = 25253) : 
--
...
WITH (oids = false);
--
-- Structure for table acreditacion_alta_calidad (OID = 25257) : 
--
SET search_path = public, pg_catalog;
...
WITH (oids = false);
--
-- Structure for table formacion (OID = 25278) : 
--
...
WITH (oids = false);
--
-- Structure for table formacion_departamento (OID = 25283) : 
--
...
WITH (oids = false);
--
-- Structure for table manuales_indicadores (OID = 25287) : 
--
...
WITH (oids = false);
--
-- Structure for table poblacion_estudiantes (OID = 25293) : 
--
...
WITH (oids = false);
--
-- Structure for table programas (OID = 25297) : 
--
...
WITH (oids = false);
--
-- Structure for table users (OID = 25302) : 
--
...
WITH (oids = false);
--
-- Definition for view vista_poblacion_docentes (OID = 25309) : 
--
...
--
-- Structure for table estudiantes_departamento (OID = 25444) : 
--
CREATE TABLE public.estudiantes_departamento (
    cod_est_dep integer DEFAULT nextval('estudiantes_departamento_cod_est_dep_seq'::regclass) NOT NULL,
    departamento char(2),
...
WITH (oids = false);
--
-- Structure for table KPI_Estudiantes_por_Docentes_TC (OID = 25467) : 
--
SET search_path = "Datawarehouse", pg_catalog;
CREATE TABLE "Datawarehouse"."KPI_Estudiantes_por_Docentes_TC" (
    "Anho" char(4) NOT NULL,
    estudiantes numeric(5,0),
    docentes numeric(3,0),
    razonanual numeric(3,0),
...
\.
--
-- Definition for index poblacion_estudiantes_promedio_key (OID = 25320) : 
--
SET search_path = public, pg_catalog;
CREATE UNIQUE INDEX poblacion_estudiantes_promedio_key ON poblacion_estudiantes USING btree (promedio);
--
-- Definition for index KPI_Desercion_Cohorte_idx (OID = 25321) : 
--
SET search_path = "Datawarehouse", pg_catalog;
...
--
-- Definition for index KPI_Formacion_fk1 (OID = 25378) : 
--
ALTER TABLE ONLY "KPI_Formacion"
    ADD CONSTRAINT "KPI_Formacion_fk1"
    FOREIGN KEY ("manual_Formacion") REFERENCES public.manuales_indicadores(codigo);
--
-- Definition for index KPI_Nivel_Satisfaccion_fk (OID = 25383) : 
--
ALTER TABLE ONLY "KPI_Nivel_Satisfaccion"
    ADD CONSTRAINT "KPI_Nivel_Satisfaccion_fk"
    FOREIGN KEY (manual) REFERENCES public.manuales_indicadores(codigo);
--
-- Definition for index KPI_Period_Dropout_fk (OID = 25388) : 
--
ALTER TABLE ONLY "KPI_Desercion_Periodo"
    ADD CONSTRAINT "KPI_Period_Dropout_fk"
    FOREIGN KEY (programa) REFERENCES public.programas(snies);
--
-- Definition for index KPI_Relacion_Docentes_fk (OID = 25393) : 
--
...
--
-- Definition for index KPI_Satisfaction_level_fk (OID = 25398) : 
--
ALTER TABLE ONLY "KPI_Nivel_Satisfaccion"
    ADD CONSTRAINT "KPI_Satisfaction_level_fk"
    FOREIGN KEY ("Programa") REFERENCES public.programas(snies);
--
-- Definition for index acreditacion_alta_calidad_fk (OID = 25403) : 
--
SET search_path = public, pg_catalog;
ALTER TABLE ONLY acreditacion_alta_calidad
...
--
-- Definition for index KPI_Estudiantes_por_Docentes_TC_pkey (OID = 25499) : 
--
ALTER TABLE ONLY "KPI_Estudiantes_por_Docentes_TC"
    ADD CONSTRAINT "KPI_Estudiantes_por_Docentes_TC_pkey"
    PRIMARY KEY ("Anho");
--
-- Definition for trigger tr_actua_satisfaccion (OID = 25425) : 
--
...
--
-- Definition for trigger tr_cohort_desertion (OID = 25426) : 
--
...
--
-- Definition for trigger tr_period_desertion (OID = 25427) : 
--
...
--
-- Definition for trigger tr_relacion (OID = 25428) : 
--
...
--
-- Definition for trigger tr_for_dep (OID = 25429) : 
--
SET search_path = public, pg_catalog;
...
--
-- Definition for trigger tr_kpi_form (OID = 25430) : 
--
...
--
-- Definition for trigger tr_est_dep (OID = 25456) : 
--
...
--
-- Definition for trigger tr_kpi_est_dep (OID = 25457) : 
--
...
--
-- Data for sequence public.formacion_departamento_cod_forma_dep_seq (OID = 25281)
--
SELECT pg_catalog.setval('formacion_departamento_cod_forma_dep_seq', 11565, true);
--
-- Data for sequence public.estudiantes_departamento_cod_est_dep_seq (OID = 25442)
--
