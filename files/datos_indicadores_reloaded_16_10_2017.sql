...
--
-- Definition for function fun_actua_formacion (OID = 28369) : 
--
...
LANGUAGE plpgsql;
--
-- Definition for function fun_actua_relacion (OID = 28370) : 
--
...
LANGUAGE plpgsql;
--
-- Definition for function fun_delete_cohort (OID = 28371) : 
--
...
LANGUAGE plpgsql;
--
-- Definition for function fun_delete_period (OID = 28372) : 
--
...
LANGUAGE plpgsql;
--
-- Definition for function Actua_Formacion_Dep (OID = 28373) : 
--
...
LANGUAGE plpgsql;
--
-- Definition for function Fun_Actua_KPI_Form (OID = 28374) : 
--
...
LANGUAGE plpgsql;
--
-- Definition for function fun_actua_satisfaccion (OID = 28375) : 
--
...
LANGUAGE plpgsql;
--
-- Definition for function Actua_Estudiantes_Dep (OID = 28376) : 
--
...
LANGUAGE plpgsql;
--
-- Definition for function Fun_Actua_KPI_Est_Dep (OID = 28377) : 
--
...
LANGUAGE plpgsql;
--
-- Definition for function Fun_Actua_KPI_Est_Doc (OID = 28378) : 
--
...
LANGUAGE plpgsql;
--
-- Structure for table KPI_Acreditacion (OID = 28379) : 
--
SET search_path = "Datawarehouse", pg_catalog;
...
WITH (oids = false);
--
-- Structure for table KPI_Desercion_Cohorte (OID = 28383) : 
--
...
WITH (oids = false);
--
-- Structure for table KPI_Desercion_Periodo (OID = 28387) : 
--
...
WITH (oids = false);
--
-- Structure for table KPI_Formacion (OID = 28391) : 
--
...
WITH (oids = false);
--
-- Structure for table KPI_Nivel_Satisfaccion (OID = 28395) : 
--
...
WITH (oids = false);
--
-- Structure for table KPI_Relacion_Docentes (OID = 28399) : 
--
...
WITH (oids = false);
--
-- Structure for table acreditacion_alta_calidad (OID = 28403) : 
--
SET search_path = public, pg_catalog;
...
WITH (oids = false);
--
-- Structure for table formacion (OID = 28424) : 
--
...
WITH (oids = false);
--
-- Structure for table formacion_departamento (OID = 28429) : 
--
...
WITH (oids = false);
--
-- Structure for table manuales_indicadores (OID = 28433) : 
--
...
WITH (oids = false);
--
-- Structure for table poblacion_estudiantes (OID = 28439) : 
--
...
WITH (oids = false);
--
-- Structure for table programas (OID = 28443) : 
--
...
WITH (oids = false);
--
-- Structure for table users (OID = 28448) : 
--
...
WITH (oids = false);
--
-- Definition for view vista_poblacion_docentes (OID = 28455) : 
--
...
--
-- Structure for table estudiantes_departamento (OID = 28462) : 
--
...
WITH (oids = false);
--
-- Structure for table KPI_Estudiantes_por_Docentes_TC (OID = 28466) : 
--
SET search_path = "Datawarehouse", pg_catalog;
...
WITH (oids = false);
--
-- Structure for table cantidad_e (OID = 28596) : 
--
SET search_path = public, pg_catalog;
CREATE TABLE public.cantidad_e (
    count bigint
)
WITH (oids = false);
--
-- Structure for table cantidad_f (OID = 28599) : 
--
CREATE TABLE public.cantidad_f (
    count bigint
)
WITH (oids = false);
--
-- Structure for table cantidad_g (OID = 28602) : 
--
CREATE TABLE public.cantidad_g (
    count bigint
)
WITH (oids = false);
--
-- Structure for table cantidad_h (OID = 28605) : 
--
CREATE TABLE public.cantidad_h (
    count bigint
)
WITH (oids = false);
--
-- Structure for table cantidad_i (OID = 28608) : 
--
CREATE TABLE public.cantidad_i (
    count bigint
)
WITH (oids = false);
--
-- Structure for table cantidad_Ñ (OID = 28611) : 
--
CREATE TABLE public."cantidad_Ñ" (
    count bigint
)
WITH (oids = false);
--
-- Structure for table cantidad_b (OID = 28614) : 
--
CREATE TABLE public.cantidad_b (
    count bigint
)
WITH (oids = false);
--
-- Structure for table cantidad_z (OID = 28617) : 
--
CREATE TABLE public.cantidad_z (
    count bigint
)
WITH (oids = false);
SET search_path = "Datawarehouse", pg_catalog;
...
1 	116	0	7	2017	2  	91.00
2 	152	11	127	2017	2  	91.00
3 	21	25	192	2017	2  	91.00
4 	6	11	172	2017	2  	91.00
\.
...
11326	1 	5	0	0	2017	1 	1 
11327	2 	2	0	0	2017	1 	1 
11328	3 	1	0	0	2017	1 	1 
11329	4 	0	0	0	2017	1 	1 
11330	5 	7	0	0	2017	1 	1 
11331	6 	6	0	1	2017	1 	1 
11332	7 	0	0	0	2017	1 	1 
11333	8 	1	0	0	2017	1 	1 
11334	9 	1	0	0	2017	1 	1 
11335	10	2	0	1	2017	1 	1 
11336	11	3	0	0	2017	1 	1 
11337	12	1	0	0	2017	1 	1 
11338	13	6	0	1	2017	1 	1 
11339	14	2	0	0	2017	1 	1 
11340	15	2	0	0	2017	1 	1 
11341	16	0	0	0	2017	1 	1 
11342	17	0	0	0	2017	1 	1 
11343	18	8	0	2	2017	1 	1 
11344	19	7	0	0	2017	1 	1 
11345	20	7	0	0	2017	1 	1 
11346	21	9	0	0	2017	1 	1 
11347	22	3	0	0	2017	1 	1 
11348	23	5	0	2	2017	1 	1 
11349	24	2	0	0	2017	1 	1 
11350	25	2	0	0	2017	1 	1 
11351	26	3	0	0	2017	1 	1 
11352	27	4	0	0	2017	1 	1 
11353	28	0	0	0	2017	1 	1 
11354	29	5	0	0	2017	1 	1 
11355	30	2	0	0	2017	1 	1 
11356	1 	3	0	3	2017	1 	2 
11357	2 	4	0	5	2017	1 	2 
11358	3 	7	0	5	2017	1 	2 
11359	4 	4	0	2	2017	1 	2 
11360	5 	3	0	7	2017	1 	2 
11361	6 	4	0	10	2017	1 	2 
11362	7 	6	0	2	2017	1 	2 
11363	8 	3	1	6	2017	1 	2 
11364	9 	1	0	1	2017	1 	2 
11365	10	4	0	3	2017	1 	2 
11366	11	7	1	3	2017	1 	2 
11367	12	1	1	1	2017	1 	2 
11368	13	11	0	7	2017	1 	2 
11369	14	5	0	7	2017	1 	2 
11370	15	4	1	1	2017	1 	2 
11371	16	3	1	3	2017	1 	2 
11372	17	1	1	3	2017	1 	2 
11373	18	4	1	3	2017	1 	2 
11374	19	4	0	8	2017	1 	2 
11375	20	18	0	10	2017	1 	2 
11376	21	3	1	5	2017	1 	2 
11377	22	4	0	0	2017	1 	2 
11378	23	7	0	15	2017	1 	2 
11379	24	5	1	1	2017	1 	2 
11380	25	8	1	7	2017	1 	2 
11381	26	4	0	0	2017	1 	2 
11382	27	4	0	1	2017	1 	2 
11383	28	0	0	0	2017	1 	2 
11384	29	8	1	5	2017	1 	2 
11385	30	3	0	3	2017	1 	2 
11386	1 	2	0	6	2017	1 	3 
11387	2 	3	0	14	2017	1 	3 
11388	3 	0	1	6	2017	1 	3 
11389	4 	1	3	5	2017	1 	3 
11390	5 	0	0	3	2017	1 	3 
11391	6 	0	1	3	2017	1 	3 
11392	7 	2	0	8	2017	1 	3 
11393	8 	2	4	9	2017	1 	3 
11394	9 	1	3	7	2017	1 	3 
11395	10	0	0	2	2017	1 	3 
11396	11	0	0	0	2017	1 	3 
11397	12	1	1	1	2017	1 	3 
11398	13	0	2	19	2017	1 	3 
11399	14	0	0	5	2017	1 	3 
11400	15	0	0	5	2017	1 	3 
11401	16	0	0	4	2017	1 	3 
11402	17	1	3	38	2017	1 	3 
11403	18	1	0	0	2017	1 	3 
11404	19	0	0	4	2017	1 	3 
11405	20	2	0	18	2017	1 	3 
11406	21	0	0	5	2017	1 	3 
11407	22	1	0	0	2017	1 	3 
11408	23	0	0	2	2017	1 	3 
11409	24	3	1	6	2017	1 	3 
11410	25	0	4	10	2017	1 	3 
11411	26	0	1	2	2017	1 	3 
11412	27	0	0	1	2017	1 	3 
11413	28	0	0	0	2017	1 	3 
11414	29	1	0	8	2017	1 	3 
11415	30	0	1	1	2017	1 	3 
11416	1 	1	0	2	2017	1 	4 
11417	2 	0	0	10	2017	1 	4 
11418	3 	0	3	20	2017	1 	4 
11419	4 	1	1	4	2017	1 	4 
11420	5 	0	0	2	2017	1 	4 
11421	6 	1	1	7	2017	1 	4 
11422	7 	0	0	1	2017	1 	4 
11423	8 	0	0	2	2017	1 	4 
11424	9 	0	1	0	2017	1 	4 
11425	10	0	0	3	2017	1 	4 
11426	11	0	0	1	2017	1 	4 
11427	12	0	0	0	2017	1 	4 
11428	13	0	1	44	2017	1 	4 
11429	14	0	0	8	2017	1 	4 
11430	15	0	0	1	2017	1 	4 
11431	16	0	1	3	2017	1 	4 
11432	17	0	1	13	2017	1 	4 
11433	18	0	0	6	2017	1 	4 
11434	19	0	0	4	2017	1 	4 
11435	20	0	0	13	2017	1 	4 
11436	21	0	0	3	2017	1 	4 
11437	22	0	0	0	2017	1 	4 
11438	23	0	0	4	2017	1 	4 
11439	24	1	0	2	2017	1 	4 
11440	25	0	0	6	2017	1 	4 
11441	26	0	1	1	2017	1 	4 
11442	27	0	0	5	2017	1 	4 
11443	28	0	0	0	2017	1 	4 
11444	29	1	0	4	2017	1 	4 
11445	30	1	1	3	2017	1 	4 
12526	1 	5	0	0	2017	2 	1 
12527	2 	2	0	0	2017	2 	1 
12528	3 	1	0	0	2017	2 	1 
12529	4 	0	0	0	2017	2 	1 
12530	5 	10	0	0	2017	2 	1 
12531	6 	10	0	1	2017	2 	1 
12532	7 	0	0	0	2017	2 	1 
12533	8 	1	0	0	2017	2 	1 
12534	9 	1	0	0	2017	2 	1 
12535	10	2	0	1	2017	2 	1 
12536	11	3	0	0	2017	2 	1 
12537	12	1	0	0	2017	2 	1 
12538	13	10	0	1	2017	2 	1 
12539	14	2	0	0	2017	2 	1 
12540	15	2	0	0	2017	2 	1 
12541	16	0	0	0	2017	2 	1 
12542	17	0	0	0	2017	2 	1 
12543	18	10	0	2	2017	2 	1 
12544	19	10	0	0	2017	2 	1 
12545	20	10	0	0	2017	2 	1 
12546	21	10	0	0	2017	2 	1 
12547	22	3	0	0	2017	2 	1 
12548	23	5	0	2	2017	2 	1 
12549	24	2	0	0	2017	2 	1 
12550	25	2	0	0	2017	2 	1 
12551	26	3	0	0	2017	2 	1 
12552	27	4	0	0	2017	2 	1 
12553	28	0	0	0	2017	2 	1 
12554	29	5	0	0	2017	2 	1 
12555	30	2	0	0	2017	2 	1 
12556	1 	3	0	3	2017	2 	2 
12557	2 	4	0	5	2017	2 	2 
12558	3 	7	0	5	2017	2 	2 
12559	4 	4	0	2	2017	2 	2 
12560	5 	3	0	7	2017	2 	2 
12561	6 	4	0	10	2017	2 	2 
12562	7 	6	0	2	2017	2 	2 
12563	8 	3	1	6	2017	2 	2 
12564	9 	1	0	1	2017	2 	2 
12565	10	4	0	3	2017	2 	2 
12566	11	7	1	3	2017	2 	2 
12567	12	1	1	1	2017	2 	2 
12568	13	20	0	7	2017	2 	2 
12569	14	5	0	7	2017	2 	2 
12570	15	4	1	1	2017	2 	2 
12571	16	3	1	3	2017	2 	2 
12572	17	1	1	3	2017	2 	2 
12573	18	4	1	3	2017	2 	2 
12574	19	4	0	8	2017	2 	2 
12575	20	18	0	10	2017	2 	2 
12576	21	3	1	5	2017	2 	2 
12577	22	4	0	0	2017	2 	2 
12578	23	7	0	15	2017	2 	2 
12579	24	5	1	1	2017	2 	2 
12580	25	8	1	7	2017	2 	2 
12581	26	4	0	0	2017	2 	2 
12582	27	4	0	1	2017	2 	2 
12583	28	0	0	0	2017	2 	2 
12584	29	8	1	5	2017	2 	2 
12585	30	3	0	3	2017	2 	2 
12586	1 	2	0	6	2017	2 	3 
12587	2 	3	0	14	2017	2 	3 
12588	3 	0	1	6	2017	2 	3 
12589	4 	1	3	5	2017	2 	3 
12590	5 	0	0	3	2017	2 	3 
12591	6 	0	1	3	2017	2 	3 
12592	7 	2	0	8	2017	2 	3 
12593	8 	2	4	9	2017	2 	3 
12594	9 	1	3	7	2017	2 	3 
12595	10	0	0	2	2017	2 	3 
12596	11	0	0	0	2017	2 	3 
12597	12	1	1	1	2017	2 	3 
12598	13	0	2	19	2017	2 	3 
12599	14	0	0	5	2017	2 	3 
12600	15	0	0	5	2017	2 	3 
12601	16	0	0	4	2017	2 	3 
12602	17	1	3	38	2017	2 	3 
12603	18	1	0	0	2017	2 	3 
12604	19	0	0	4	2017	2 	3 
12605	20	2	0	18	2017	2 	3 
12606	21	0	0	5	2017	2 	3 
12607	22	1	0	0	2017	2 	3 
12608	23	0	0	2	2017	2 	3 
12609	24	3	1	6	2017	2 	3 
12610	25	0	4	10	2017	2 	3 
12611	26	0	1	2	2017	2 	3 
12612	27	0	0	1	2017	2 	3 
12613	28	0	0	0	2017	2 	3 
12614	29	1	0	8	2017	2 	3 
12615	30	0	1	1	2017	2 	3 
12616	1 	1	0	2	2017	2 	4 
12617	2 	0	0	10	2017	2 	4 
12618	3 	0	3	20	2017	2 	4 
12619	4 	1	1	4	2017	2 	4 
12620	5 	0	0	2	2017	2 	4 
12621	6 	1	1	7	2017	2 	4 
12622	7 	0	0	1	2017	2 	4 
12623	8 	0	0	2	2017	2 	4 
12624	9 	0	1	0	2017	2 	4 
12625	10	0	0	3	2017	2 	4 
12626	11	0	0	1	2017	2 	4 
12627	12	0	0	0	2017	2 	4 
12628	13	0	1	44	2017	2 	4 
12629	14	0	0	8	2017	2 	4 
12630	15	0	0	1	2017	2 	4 
12631	16	0	1	3	2017	2 	4 
12632	17	0	1	13	2017	2 	4 
12633	18	0	0	6	2017	2 	4 
12634	19	0	0	4	2017	2 	4 
12635	20	0	0	13	2017	2 	4 
12636	21	0	0	3	2017	2 	4 
12637	22	0	0	0	2017	2 	4 
12638	23	0	0	4	2017	2 	4 
12639	24	1	0	2	2017	2 	4 
12640	25	0	0	6	2017	2 	4 
12641	26	0	1	1	2017	2 	4 
12642	27	0	0	5	2017	2 	4 
12643	28	0	0	0	2017	2 	4 
12644	29	1	0	4	2017	2 	4 
12645	30	1	1	3	2017	2 	4 
\.
...
\.
SET search_path = public, pg_catalog;
COPY cantidad_e (count) FROM stdin;
0
\.
COPY cantidad_f (count) FROM stdin;
0
\.
COPY cantidad_g (count) FROM stdin;
0
\.
COPY cantidad_h (count) FROM stdin;
0
\.
COPY cantidad_i (count) FROM stdin;
0
\.
COPY "cantidad_Ñ" (count) FROM stdin;
0
\.
COPY cantidad_b (count) FROM stdin;
0
\.
COPY cantidad_z (count) FROM stdin;
0
\.
--
-- Definition for index poblacion_estudiantes_promedio_key (OID = 28471) : 
--
CREATE UNIQUE INDEX poblacion_estudiantes_promedio_key ON poblacion_estudiantes USING btree (promedio);
--
-- Definition for index KPI_Desercion_Cohorte_idx (OID = 28472) : 
--
SET search_path = "Datawarehouse", pg_catalog;
...
--
-- Definition for index KPI_Formacion_fk1 (OID = 28519) : 
--
ALTER TABLE ONLY "KPI_Formacion"
    ADD CONSTRAINT "KPI_Formacion_fk1"
    FOREIGN KEY ("manual_Formacion") REFERENCES public.manuales_indicadores(codigo);
--
-- Definition for index KPI_Nivel_Satisfaccion_fk (OID = 28524) : 
--
ALTER TABLE ONLY "KPI_Nivel_Satisfaccion"
    ADD CONSTRAINT "KPI_Nivel_Satisfaccion_fk"
    FOREIGN KEY (manual) REFERENCES public.manuales_indicadores(codigo);
--
-- Definition for index KPI_Period_Dropout_fk (OID = 28529) : 
--
ALTER TABLE ONLY "KPI_Desercion_Periodo"
    ADD CONSTRAINT "KPI_Period_Dropout_fk"
    FOREIGN KEY (programa) REFERENCES public.programas(snies);
--
-- Definition for index KPI_Relacion_Docentes_fk (OID = 28534) : 
--
...
--
-- Definition for index KPI_Satisfaction_level_fk (OID = 28539) : 
--
ALTER TABLE ONLY "KPI_Nivel_Satisfaccion"
    ADD CONSTRAINT "KPI_Satisfaction_level_fk"
    FOREIGN KEY ("Programa") REFERENCES public.programas(snies);
--
-- Definition for index acreditacion_alta_calidad_fk (OID = 28544) : 
--
SET search_path = public, pg_catalog;
ALTER TABLE ONLY acreditacion_alta_calidad
...
--
-- Definition for trigger tr_actua_satisfaccion (OID = 28576) : 
--
...
--
-- Definition for trigger tr_cohort_desertion (OID = 28577) : 
--
...
--
-- Definition for trigger tr_period_desertion (OID = 28578) : 
--
...
--
-- Definition for trigger tr_relacion (OID = 28579) : 
--
...
--
-- Definition for trigger tr_for_dep (OID = 28580) : 
--
SET search_path = public, pg_catalog;
...
--
-- Definition for trigger tr_kpi_form (OID = 28581) : 
--
...
--
-- Definition for trigger tr_kpi_est_dep (OID = 28582) : 
--
...
--
-- Definition for trigger tr_est_dep (OID = 28583) : 
--
...
--
-- Definition for trigger tr_kpi_est_doc (OID = 28584) : 
--
...
ALTER TABLE estudiantes_departamento
  DISABLE TRIGGER tr_kpi_est_doc;
--
-- Data for sequence public.formacion_departamento_cod_forma_dep_seq (OID = 28427)
--
SELECT pg_catalog.setval('formacion_departamento_cod_forma_dep_seq', 12645, true);
--
-- Data for sequence public.estudiantes_departamento_cod_est_dep_seq (OID = 28460)
--
SELECT pg_catalog.setval('estudiantes_departamento_cod_est_dep_seq', 27063, true);
--
-- Comments
--
